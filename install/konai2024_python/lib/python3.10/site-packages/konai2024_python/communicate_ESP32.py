...
battery_voltage:list = [0,0] # 4セル,3セルの順にバッテリー電圧を代入(単位:V)
...
def serial_reception():
    ...
    while True:
        try:
            ...
            received_message_array_1.split(",")
            received_message_array_1 = [int(x) for x in received_message_array_1] # 文字列を数値化
            if received_message_1[0] == 1: # 4セルバッテリー
                battery_voltage[0] = received_message_1[1]
            elif received_message_1[0] == 2: #3セルバッテリー
                battery_voltage[1] = received_message_1[1]
        ...
        try:
            ...
            received_message_array_2.split(",")
            received_message_array_2 = [int(x) for x in received_message_array_2] # 文字列を数値化
            if received_message_2[0] == 1: # 4セルバッテリー
                battery_voltage[0] = received_message_2[1]
            elif received_message_2[0] == 2: #3セルバッテリー
                battery_voltage[1] = received_message_2[1]
        except UnicodeDecodeError as e:
