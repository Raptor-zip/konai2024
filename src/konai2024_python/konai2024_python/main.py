...
micon_dict: dict[str, dict] = {
    "ESP32": {"is_connected": False,  # パソコンと接続しているか
                "serial_id": None, # ポート 例:/dev/ttyUSB0
...
def recept_serial():
    global micon_dict, serial_reception_text
...
def recept_serial_2():
    ...
    while True:
        each_micon_dict_key = "STM32"
        each_micon_dict_values = micon_dict["STM32"]
        try:
            received_message: str = each_micon_dict_values["serial_obj"].readline(
            ).decode('utf-8')[:-2]  # \r\nを消す
            print(
...
class MinimalSubscriber(Node):
    ...
    def timer_callback_0033(self):
        ...
        temp_micon_dict: dict = {"ESP32": {
            ...
            "serial_id": micon_dict["ESP32"]["serial_id"]
        },
            "STM32": {
            "is_connected": micon_dict["STM32"]["is_connected"],
            "serial_id": micon_dict["STM32"]["serial_id"]
        }, }
