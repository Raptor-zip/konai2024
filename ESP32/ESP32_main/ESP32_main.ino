...
// 距離センサー
const uint8_t sensor_count = 4; // センサーの個数
const uint8_t PIN_SER   =  2; // TODO 配列か構造体か何かにする
const uint8_t PIN_LATCH =  4;
const uint8_t PIN_CLK   = 15;
byte patterns[] = {
  B00000000,
  B10000000,
  B11000000,
...
void setup() {
  ...
  // VESCモーターの初期化
    Serial1.begin(115200, SERIAL_8N1, 19, 18, false, 100); // RX,TXの順 // https://github.com/espressif/arduino-esp32/blob/master/cores/esp32/HardwareSerial.cpp#L280
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (incomingStrings.length() > 5) {
      int16_t intArray[maxElements];   // 整数の配列
      uint8_t count = parseStringToArray(incomingStrings, intArray, maxElements);
...
uint8_t parseStringToArray(String input, int16_t array[], uint8_t maxSize) {
  uint8_t count = 0;
